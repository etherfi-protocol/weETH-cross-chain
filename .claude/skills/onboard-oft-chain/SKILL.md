...
- **Media kit (chain logo) — ask the user every time, do not guess or reuse another chain's art.** The FE PR (step 12) needs an svg/png/webp network icon. There is no registry field or API for this and chain docs generally don't carry one: Arc's public docs (`docs.arc.io`, whose `/llms.txt` is a complete page index) and its private-mainnet page both document chain config in detail and have **no brand/logo/press page at all**. So: read the chain's docs for the *config*, then ask the user "Where's the media kit / logo for `<chainKey>`?" for the *art* — and stop rather than substitute a placeholder or another chain's icon. The chain's own integrator docs are still the best source for the config the FE needs (Arc: `docs.arc.io/integrate/connect-to-arc` gives a ready-made `defineChain` block, the public RPC `https://rpc.mainnet.arc.io`, explorer `https://explorer.arc.io`, and an Alchemy endpoint `https://arc-mainnet.g.alchemy.com/v2/<key>`).
...
- Ends with **explorer verification** of every deployed contract, then **FE/BE chain-registry PRs** to `cash-be` and `turbo-neobank` (step 12) so the chain shows up in the Cash app — these need the media kit asked for above — and a **public docs PR** to `etherfi-protocol/docs` (step 13). A chain is not "onboarded" until all three land.
...
12. **FE/BE chain-registry PRs (cash-be + turbo-neobank) — do this for every onboarding, not just as a follow-up.** A chain deployed and wired on-chain is invisible to users until `cash-be` and `turbo-neobank` know about it. Robinhood's pair is the template: [`cash-be#6486`](https://github.com/etherfi-protocol/cash-be/pull/6486) and [`turbo-neobank#3900`](https://github.com/etherfi-protocol/turbo-neobank/pull/3900). Open both as separate PRs in their own repos/clones (neither lives in this repo):
    - **`cash-be`** (`src/chain-registry/`): add `configs/<chainKey>.config.ts` — a thin `ChainConfig` (`chainId`, `chainName`, `displayName`, `alchemyName`, `viemChain`, `contracts: { production: null, development: null }` since a mesh-only chain doesn't run Cash protocol, `tokens`/`tokenMetadata` from `generateChainTokenConfig(<chain>.id, [SupportedToken.WEETH], false)`, `explorerUrl`, `bridges.layerZeroEid`, `supportsCashProtocol: false`); export it from `configs/index.ts` and add to `ALL_CHAIN_CONFIGS`; if the chain isn't in `viem/chains`, add a local `defineChain` in `constants/index.ts` and register it in `CHAIN_ID_TO_CHAIN` — **and if it IS in `viem/chains`, still check the definition is not a stub.** viem 2.56.5 ships `arc` (id 5042, `nativeCurrency` USDC/18) with **`rpcUrls.default.http: []` and no `blockExplorers` at all**, so importing it and reading `chain.blockExplorers.default.url` for `explorerUrl` yields `undefined` and any transport built from the chain's own RPC list has nowhere to go. Extend the imported chain with the real RPC + explorer rather than trusting it wholesale (for Arc: `https://rpc.mainnet.arc.io`, `https://explorer.arc.io`); add the weETH OFT address to `TOKEN_REGISTRY`'s `weeth` entry and to `buildWeethBridgeRoutes()` — **a new mesh chain with a restricted peer set (e.g. only Eth/Base/OP, not the full free mesh) is a limited-peer entry, not a join to the live mesh**; extend `token-registry.bridge-routes.spec.ts` for the new topology.
    - **`turbo-neobank`** (`apps/cash/`): add the network icon in all three formats under `public/app/cash/images/networks/{svg,png,webp}/<chainKey>.{svg,png,webp}` from the **media kit asked for in step 1** — never substitute a placeholder; add a `networks.<chainKey>` entry in `src/constants/network.ts` (`defineChain` with the chain's **real `nativeCurrency`** — check this per chain, e.g. Arc's is USDC at 18 decimals, not ETH — RPC, explorer, LayerZero `eid`) and import the icon; add the chain to `STARGATE_CHAIN_NAMES` in `send-eoa-flow.tsx` if it's a Stargate-routed destination; extend `send-eoa-flow.test.tsx`'s bridge-route fixture for the new topology.

    Both PRs are config-only and additive (no auth/payment logic touched); open them as normal PRs against each repo's default branch, following that repo's own PR conventions — this is unrelated to the `3CP-secure` governance flow. Track them as separate Linear sub-tickets under the chain's onboarding ticket: one to draft the PRs, one to get them reviewed/merged/deployed to production.
13. **Public docs PR (`etherfi-protocol/docs`) — every chain, no exceptions.** The GitBook docs carry the canonical public address list, and a chain that is live but undocumented is one integrators cannot find. Add the new chain to the **weETH table** in `contracts-and-integrations/deployed-contracts.md`: one `<tr>` in the big HTML table, kept in the existing alphabetical position, linking the OFT address to that chain's explorer. Branch `docs/<chain>-weeth-oft`, subject `Deployed contracts: <Chain> weETH OFT`, PR against `main` (Arc's is #39; Robinhood's row is the precedent).
    **Scope it deliberately — one row is usually the whole job.** Before adding anything else, check and rule out the other candidates, because the wrong addition is worse than a missing one:
    - `getting-started/multichain-ecosystem/cross-chain-native-staking.md` lists chains for **native minting only**. A mesh-only chain does **not** belong there; adding it advertises a mint path that does not exist.
    - `getting-started/multichain-ecosystem/README.md` is generic prose with no chain list — nothing to change.
    - `security-and-risks/multisigs-and-controls/` documents **mainnet governance only**. No per-chain L2 Safe or timelock is published for any chain, so do not add the new chain's controller Safe or timelock; it would be inconsistent with every other chain and implies a disclosure standard the docs do not keep.
    Verify the explorer link actually resolves (HTTP 200) before opening the PR — a dead address link in public docs is worse than no row. Do this **after** step 8, so the contracts are explorer-verified by the time anyone follows the link.
...
**An explorer behind Cloudflare: send the key as an `x-api-key` HEADER, not a query param.** Arc's Blockscout (`explorer.arc.io`) sits behind Cloudflare, which answers **403 `cf-mitigated: challenge`** to anything it doesn't like — *before* the request reaches Blockscout. So the etherscan-compatible `?apikey=` query param and the `apikey` form field, which are what authenticate to Blockscout itself, never arrive. Measured on arc: `?apikey=` → 403, `?api_key=` → 403, `Authorization: Bearer` → 403, **`x-api-key: <key>` → 200**. `verify-direct.mjs` now sends the header on all three call sites (pre-check, submit, status poll); pass the key via `--apikey` or `EXPLORER_API_KEY`.
**Corollary: `cf-access-proxy.mjs` was not needed for arc.** It is built around a `CF_Authorization` cookie / Access service token and its docstring describes a 302 to `circle.cloudflareaccess.com`. That is no longer the gate's behaviour — with the API-key header, direct requests work and no localhost proxy is required. Try the header first; reach for the proxy only if a chain genuinely redirects to an Access login.
